...
// Helper: select PermMmaTileN layout based on TileN. See sm120/mxfp8/utils.cuh
// for the rationale — same C9 bug applied here: a size-128 PermMmaTileN used
// with TileN=64 writes R2S register fragments past SmemLayoutD's TileN=64
// allocation and trips cudaErrorMisalignedAddress at runtime. TileN=64 must
// use a true size-64 layout so the 4 N-warps cover exactly 4×16 = 64 columns.
template <int TileN>
struct Fp8PermMmaTileNForTileN;
template <> struct Fp8PermMmaTileNForTileN<128> { using type = Layout<Shape<_8, _4, _4>, Stride<_1, _32, _8>>; };
template <> struct Fp8PermMmaTileNForTileN<64>  { using type = Layout<Shape<_8, _4, _2>, Stride<_1, _16, _8>>; };

// `MinBlocksPerSm_` defaults to 1 to keep existing instantiations byte-identical.
// P3: add a `(32, 128, 2, /*MinBlocksPerSm=*/2)` instance for small-M shapes
// where the kernel is SM-saturation-bound (latency floor, not arithmetic
// intensity) — doubling occupancy via __launch_bounds__ should lift the floor.
...
struct SM120BlockScaledBuilder
{
    ...
    // E29 (2026-05-12): persistent-scheduler swizzle group size. FP8
    // (1×128) path keeps the historical 16; only the MXFP8 builder
    // exposes a non-default override.
    static constexpr int kSchedGroup = 16;
    static constexpr int kSFVecSize = 128;
    // 2026-05-06 experiment: tried kSFVecSize=32 (true OCP MXFP8 spec — the
    // hw mxf8f6f4 atom is SM120_16x8x32_TN_VS<..., VS=32> so 32 is the
    // hardware-native granularity; 128 here is a software aggregation).
    // CUTLASS template compiles fine, kernel runs, but on random N(0,1)
    // weights the per-block cos sim (vs bf16 ref) drops 0.979 → 0.961.
    // Reason: at uniform-Gaussian distribution per-32-block amax is only
    // ~15% tighter than per-128-block amax, while UE8M0 round-up adds
    // up to a 2× scale error per block, applied 4× more often with
    // kSFVecSize=32. Net: ~4× more independent rounding variance > the
    // amax-tightening benefit. With outlier-heavy trained weights the
    // ranking should flip (outliers concentrate in few blocks; finer
    // scales isolate them). Re-test with real model weights before
    // shipping kSFVecSize=32. See `quant_1x32.py` and the
    // `FSO_BLOCK_K=32` env gate in `bsgemm_linear.py` for the
    // matching activation/weight quant path.
    ...
    // PermMmaTileN selected via Fp8PermMmaTileNForTileN<TileN_>; see helper.
    ...
    // ThrLayoutVMNK is a dependent type (TiledMma depends on the template
    // parameter PermMmaTileN_); needs `typename` to disambiguate from a
    // dependent value.

// Paged-prefill per-D TU for kHeadDim = 128.
// Per-D tile config: Br=64, Bc=32, kStages=2, kCtasPerSm=1.
// smem ≈ K(32×128×2=8KB) + V(128×32×2=8KB) + P(64×32=2KB) + scales ≈ 18.5 KB.
    ...
    cudaStream_t stream)
{
    ...
    if (causal) {
        return launch_paged_prefill_impl<128, 64, 32, 2, 1, true>(
        ...
        return launch_paged_prefill_impl<128, 64, 32, 2, 1, false>(
            Q, Qs, K_pool, K_chan_scale, V_pool, V_chan_scale,
            qo_indptr, paged_kv_indices, paged_kv_indptr,
            paged_kv_last_page_len, work_units, O,
            total_work, num_q_heads, num_kv_heads, page_size,

...
* **MXFP8 1×32** (OCP MXFP8) — 1 UE8M0 byte per 32 K-elements on both
  operands. **sm_120 only**. Finer block-scale granularity → tighter
  quantization (cos ≈ 0.9993 vs 0.987–0.996 for 1×128) and ~20–30%
  faster than 1×128 on large M.
...
| `quantize_1x32_fp8(x)` | bf16 `[..., K]`, K%128==0 | (fp8_e4m3 `[..., K]`, int32 K-major `[pad(M,4), K/128]`) | **sm_120 only**; UE8M0 baked in |
| `linear_mxfp8(x_fp8, w_fp8, sx_int32, sw_int32)` | fp8 + int32-packed UE8M0 scales | bf16 `[M, N]` | **sm_120 only**; both operands MXFP8 1×32 |
...
- `K % 128 == 0` everywhere.
- `N % 128 == 0` for `linear_fp8` and `linear_mxfp8` on sm_120.
- **FP8 1×128 on sm_120:** `use_ue8m0=True` is mandatory; without it the
  CUTLASS kernel returns NaN.
- **MXFP8 1×32:** sm_120-only. Calling on sm_90 raises
  `NotImplementedError`.

...
def _clock_warm_spin():
    """Optional DVFS settle: busy tensor-core work before any timing.

    On machines without a clock lock (e.g. b300-01: no sudo, SM clock idles
    at 120 MHz between subprocess cells and ramps to the flat 2032 MHz max
    under load), the standard 15-iteration warmup is not always enough to
    finish the ramp, which pollutes the timed medians by 5-15% on large
    cells. FSO_BENCH_WARM_MS=<ms> runs a dummy matmul spin for that long
    before the cell's timing begins. Default 0 → protocol identical to the
    locked-clock machines (5090/PRO6000/H200) so their baselines stay
    comparable.
    """
    import os, time
...
def bench_one(M, N, K, sm_major, inject_outliers: bool = False):
    ...
    # MXFP8 1×32 (sm_100/sm_103 tcgen05 + sm_120 consumer; H200 row stays
    ...
    # cuBLAS MXFP8 via torch.nn.functional.scaled_mm (sm_100/103/120; sm_90
    # raises CUBLAS_STATUS_NOT_SUPPORTED). Quantize via PyTorch's reference
    # to_mxfp + to_blocked (cuBLAS d-block-scaling layout); same numerical FP8
    # input as the FSO MXFP8 path so the comparison is purely kernel speed.
    if sm_major in (10, 12):
        try:

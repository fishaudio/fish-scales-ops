...
a markdown table (legacy `--format`; the docs/perf/ tables are generated by render_perf_docs.py).
...
FAMILIES = {
    ...
    # intermediate=9728. `gate` is a legacy row kept for continuity.
    "qwen3-4b": [
        ("wqkv",    6144,  2560),
        ("wo",      2560,  4096),
        ("gate_up", 19456, 2560),
    ...
    # Family B — Qwen3-30B-A3B dense projections (hidden=2048, 32 Q-heads +
    # 4 KV-heads, head_dim=128 → WQKV out = (32 + 2*4)*128 = 5120; WO in = 4096).
    "qwen3-30a3": [
        ("wqkv", 5120, 2048),
        ("wo",   2048, 4096),
    ],
    # Family C — Qwen3.5-35B-A3B dense projections (hidden=2048).
    # full-attention layers (10/40): 16 Q-heads × head_dim 256 with output gate
    # → q+gate 8192 + k 512 + v 512 = 9216; WO in = 16*256 = 4096.
    # Gated DeltaNet layers (30/40): in_proj q 2048 + k 2048 + v 4096 + z 4096
    # = 12288; out_proj in = 32*128 = 4096. Shared expert: moe_inter 512.
    "qwen3.5-35a3": [
        ("wqkv_gated",     9216,  2048),
        ("wo",             2048,  4096),
        ("gdn_in_proj",    12288, 2048),
        ("gdn_out_proj",   2048,  4096),
        ("shared_gate_up", 1024,  2048),
        ("shared_down",    2048,  512),
    ],
}
FAMILY = "qwen3-4b"
SHAPES = FAMILIES[FAMILY]
RUN_CUBIC = True   # the cubic peak-MFU sweep belongs to the default family only


def set_family(name):
    global FAMILY, SHAPES, RUN_CUBIC
    FAMILY = name
...
def run_grid(out_path, inject_outliers: bool = False):
    ...
    print(f"# Device: {name} (sm_{sm_major}{sm_minor})  family: {FAMILY}  cells: {len(cells)}"
    ...
    with open(out_path, "w") as f:
        f.write(json.dumps({"_device": name, "_sm": sm_major * 10 + sm_minor,
                            "_family": FAMILY, "_outliers": inject_outliers}) + "\n")
...
def format_table(jsonl_paths_with_label):
    """Legacy renderer of the old single perf.md from one-or-more jsonl files (one per device);
    the maintained tables are produced by render_perf_docs.py from tests/baselines/.
